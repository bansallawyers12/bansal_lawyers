...
@section('seoinfo')
    ...
    <?php }  else { ?>
        <title>{{ @$pagedata->title ?? 'Practice Area' }}</title>
    <?php } ?>

    <?php if( isset($pagedata->meta_description) && $pagedata->meta_description != "") { ?>
        <meta name="description" content="{{@$pagedata->meta_description}}" />
    <?php }  else { ?>
        <meta name="description" content="{{ Str::limit(strip_tags(@$pagedata->content ?? ''), 150) }}" />
    <?php } ?>
    ...
    <meta property="og:description" content="{{@$pagedata->meta_description ?? Str::limit(strip_tags(@$pagedata->content ?? ''), 150)}}">
