...
class SendAppointmentReminders extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("Sending appointment reminders for appointments in {$hours} hours...");
        ...
        $appointments = Appointment::where('status', 10) // Confirmed appointments
            ...
            ->where('reminder_sent', false) // Only send if not already sent
            ->get();
        ...
        foreach ($appointments as $appointment) {
            try {
                ...
                $this->line("✓ Reminder sent for appointment ID: {$appointment->id} - {$appointment->full_name}");
                $successCount++;
        ...
        $this->info("Reminder sending completed. Success: {$successCount}, Failures: {$failureCount}");
